...
public class EnemyAi : MonoBehaviour
{
    ...
    private void FixedUpdate()
    {
        ...
        if (cdTime02<= 0) 
        {
            ...
            Destroy(enemyFire,5f);
            cdTime02 = Random.Range(0.1f,1f);
        }
        else
        {
            cdTime02 -= 0.02f;
    ...
    void Turn()
    {
        ...
        transform.rotation = Quaternion.Slerp(transform.rotation, rotation, rtDamp * Time.deltaTime);
    }
    ...
    void LootDrop()
    {
        ...
        switch (rng) 
        {
            case <= 0.000000001f: GameManager.Instance.buffCount += 99;// BUY A LOTTERY PLS
                break;

            case <= 5: GameObject spawn = Instantiate(legendary,transform.position,Quaternion.identity); Destroy(spawn,300f);
                break;

            case <= 10: GameObject spawn2 = Instantiate(epic, transform.position, Quaternion.identity); Destroy(spawn2, 300f);
                break;

            case <= 40: GameObject spawn3 = Instantiate(rare, transform.position, Quaternion.identity); Destroy(spawn3, 300f);
                break;
            case <= 100: GameObject spawn4 = Instantiate(common, transform.position, Quaternion.identity); Destroy(spawn4, 300f);
                break;
        }
    }
}
